...
<head>
    <title>الامتحان التجريبي 2022</title><link rel="shortcut icon" type="image/x-icon" href="images/BG.png" />
    <!-- Required meta tags -->
    <meta charset="utf-8">
...
<header class="bg-gradient">
    <div class="container mt-5">
        <h1 class="arbaic-text-title" style="padding-bottom: 20px"> <strong>الامتحان التجريبي</strong> </h1>
    </div>
</header>
...
<?php
    ...
    $arrayOfFinalCorrectAnswers = []; $arrayOfFinalCorrectAnswers = $_SESSION['arrayOfFinalCorrectAnswers'];
    $arrayOfSolvedQuestionsNumbers = []; $arrayOfSolvedQuestionsNumbers = $_SESSION['arrayOfSolvedQuestionsNumbers'];
    $score = 0;
if(isset($_POST['submit']))
{
    ...
    {
        if(isset($_POST['q'.$i]))
            $arrayOfStudentAnswers[$i-1] = (int)substr($_POST['q'.$i],0,1);
        else
            $arrayOfStudentAnswers[$i-1] = 0;
    }
    
    for($i=0;$i<$numberOfQuestions;$i++)
    {
        if($arrayOfFinalCorrectAnswers[$i]==$arrayOfStudentAnswers[$i])
        {
            $score++;
        }
    }
    ...
    $score = ceil($maxTotalScore*$score/$numberOfQuestions);
    ...
    for($i=0;$i<$numberOfQuestions;$i++)
    {
        ...
        $sql = $sql . ';' . "INSERT INTO `studentdemoexamanswers`(`ClassID`, `Code`, `CorrectChoice`, `QuestionNumber`, `StudentChoice`) VALUES (?,?,?,?,?)";
        $stmtinsert = $conn->prepare($sql);
        $result = $stmtinsert->execute([$classNumber,$_COOKIE['player-code'], $arrayOfFinalCorrectAnswers[$i], $arrayOfSolvedQuestionsNumbers[$i], $arrayOfStudentAnswers[$i]]);
        $stmtinsert->closeCursor();
    }

    $sql = "UPDATE studentdemoexamresults SET DemoExamScore = ".$score.", Submission_Time = '".date('Y/m/d H:i:s', time())."' WHERE Code= ".$_COOKIE['player-code']." AND ClassID = ".$classNumber.";";
...
///INSERT Answers into Database and Update Score
...
<div class=" d-flex center flex-column" style="padding: 80px; align-items: center">
    <h2 class="arbaic-text-small-nav" style="text-align:center;font-size: xx-large; color: #810085; text-align: center">تم تسليم إجابات الامتحان التجريبي بنجاح! شكراً لك!</h2>
    <h2 class="arbaic-text-small-nav" style="text-align:center;font-size: xx-large; color: #004085; text-align: center">لقد حصلت على : <?php echo $score;?> من <?php echo $maxTotalScore?></h2>
        <a href="userpage.php" class="btn btn-primary2" style="padding: 15px; margin: 10px; color: #FFFFFF">العودة لصفحتي الشخصية</a>
