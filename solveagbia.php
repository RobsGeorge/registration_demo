...
$sql2 = "SELECT * FROM `studentagbiaexamresults` WHERE `Code`= ".$_COOKIE['player-code'];
$res2 = $conn->query($sql2);
if($res2->rowCount()>0) ////CODE FOUND IN DATABASE/////
{
        header('Location: agbiaretrial.php');
        exit();
}
else /////the first time for the student to take the exam
{
    ...
    $sql = $sql . ';' . "INSERT INTO `studentagbiaexamresults`(`Code`, `ClassID`, `AgbiaExamScore`, `Submission_Time`) VALUES (?,?,?,?)";
...
$sql2 = "SELECT NumberOfQuestions, ExamDuration FROM `agbiaexams` WHERE ClassID=".$classNumber;
$res2 = $conn->query($sql2);
if($res2->rowCount()>0) ////CODE FOUND IN DATABASE/////
{
$row2 = $res2->fetch();
...
$sql = "SELECT QuestionNumber, QuestionContent, QuestionType, CorrectChoice, FirstChoice, SecondChoice, ThirdChoice, FourthChoice FROM agbiaquestions WHERE ClassID=".$classNumber;
$res = $conn->query($sql);
...
<header class="bg-gradient">
    <div class="container mt-5">
        <h1 class="arbaic-text-title"> <strong>حل وتسليم امتحان الأجبية</strong> </h1>
        <p class="tagline" style="padding-bottom: 20px;">يجب عليك حل أكثر الأسئلة الممكنة بشكل صحيح وفي أسرع وقت ممكن للحصول على النقاط الكاملة لامتحان الأجبية</p>
    </div>
</header>
...
<div class="section bg-new-gradient">
    <form action= "confirmAgbia.php" method="post" id="quiz" dir="rtl">
    ...
    var interval = setInterval(function(){
        ...
        if (count === 0){
            ...
            Swal.fire({
                ...
                onClose: () => {
                    window.location.replace('confirmAgbia.php');
                }
            }).then((result) => {
