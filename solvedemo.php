...
$sql2 = "SELECT * FROM `studentdemoexamresults` WHERE `Code`= ".$_COOKIE['player-code'];
$res2 = $conn->query($sql2);
if($res2->rowCount()>0) ////CODE FOUND IN DATABASE/////
{
        header('Location: demoretrial.php');
        exit();
}
else /////the first time for the student to take the exam
{
    ...
    $sql = $sql . ';' . "INSERT INTO `studentdemoexamresults`(`Code`, `ClassID`, `DemoExamScore`, `Submission_Time`) VALUES (?,?,?,?)";
...
$sql2 = "SELECT NumberOfQuestions, ExamDuration FROM `demoexams` WHERE ClassID=".$classNumber;
$res2 = $conn->query($sql2);
if($res2->rowCount()>0) ////CODE FOUND IN DATABASE/////
{
$row2 = $res2->fetch();
...
$sql = "SELECT QuestionNumber, QuestionContent, QuestionType, CorrectChoice, FirstChoice, SecondChoice, ThirdChoice, FourthChoice FROM demoquestions WHERE ClassID=".$classNumber;
$res = $conn->query($sql);
...
<head>
    <title>امتحان تجريبي 2022</title><link rel="shortcut icon" type="image/x-icon" href="images/BG.png" />
    <!-- Required meta tags -->
    <meta charset="utf-8">
...
<header class="bg-gradient">
    <div class="container mt-5">
        <h1 class="arbaic-text-title"> <strong>حل وتسليم الامتحان التجريبي</strong> </h1>
        <p class="tagline" style="padding-bottom: 20px;">يجب عليك حل أكثر الأسئلة الممكنة بشكل صحيح وفي أسرع وقت ممكن للحصول على النقاط الكاملة للامتحان</p>
    </div>
</header>
...
<div class="section bg-new-gradient">
    <form name="submitform" action= "confirmdemo.php" method="post" id="quiz" dir="rtl">
    ...
    var interval = setInterval(function(){
        ...
        if (count === 0){
            ...
            Swal.fire({
                ...
                onClose: () => {
                    window.location.replace('confirmdemo.php');
                }
            }).then((result) => {
