...
$sql2 = "SELECT * FROM `studentcopticexamresults` WHERE `Code`= ".$_COOKIE['player-code'];
$res2 = $conn->query($sql2);
if($res2->rowCount()>0) ////CODE FOUND IN DATABASE/////
{
        header('Location: copticRetrial.php');
        exit();
}
else /////the first time for the student to take the exam
{
    ...
    $sql = $sql . ';' . "INSERT INTO `studentcopticexamresults`(`Code`, `ClassID`, `CopticExamScore`, `Submission_Time`) VALUES (?,?,?,?)";
...
$sql2 = "SELECT NumberOfQuestions, ExamDuration FROM `copticexams` WHERE ClassID=".$classNumber;
$res2 = $conn->query($sql2);
if($res2->rowCount()>0) ////CODE FOUND IN DATABASE/////
{
$row2 = $res2->fetch();
...
$sql = "SELECT QuestionNumber, QuestionContent, QuestionType, CorrectChoice, FirstChoice, SecondChoice, ThirdChoice, FourthChoice FROM copticquestions WHERE ClassID=".$classNumber;
$res = $conn->query($sql);
...
<header class="bg-gradient">
    <div class="container mt-5">
        <h1 class="arbaic-text-title"> <strong>حل وتسليم امتحان القبطي</strong> </h1>
        <p class="tagline" style="padding-bottom: 20px;">يجب عليك حل أكثر الأسئلة الممكنة بشكل صحيح وفي أسرع وقت ممكن للحصول على النقاط الكاملة لامتحان القبطي</p>
    </div>
</header>
...
<div class="section bg-new-gradient">
    <form action= "confirmCoptic.php" method="post" id="quiz" dir="rtl">
        <div style="margin-top: 25px;">
            <h3 class="arbaic-text-small-heading" style="text-align: center; font-size: 27px">
...
$s = $arrayOfQuestions[$randNum];
$target = strpos($s,$key);
$startCoptic = 0;

while(is_numeric($target))
{	
	if(!$startCoptic)
    {
    	$arabicWord = substr($s,0,$target);
        $copticWord = "";
        $startCoptic = 1;
    }
    else
    {
    	...
    	$copticWord = substr($s,0,$target);
        $arabicWord = "";
        $startCoptic = 0;
    }
    $s = substr($s,$target+2);
    $target = strpos($s,$key);
    if(strlen($copticWord)!=0)
    	echo "<h2 class='coptic-font-h2' style='font-size: 25px; color: #af09d5;' dir='ltr'>". $copticWord. " </h2>";
	if(strlen($arabicWord)!=0)
    	echo "<h3 style='font-size: 25px'>". $arabicWord. " </h3>";

}
	echo "<h3 style='font-size: 25px'>". $s. " </h3></div>";
                ...
                {   
                    ...
                    {	
                        ...
                        $target = strpos($s,$key);
                        if(strlen($copticWord)!=0)
                            echo "<label class='coptic-font-h2' style='font-size: 25px; color: #af09d5;' dir='ltr' >". $copticWord. " </label>";
                        if(strlen($arabicWord)!=0)
                            echo "<label style='font-size: 25px; color: #af09d5;' dir='ltr'>". $arabicWord. " </label>";

                    }
                        echo "<label style='font-size: 25px; color: #af09d5;' dir='ltr'>". $s. " </label>";
                }
                echo "</li>";
            }
...
<script>
    var selected = document.querySelector('input[name="q1"]:checked').value;
    ...
    var interval = setInterval(function(){
        ...
        if (count === 0){
            ...
            Swal.fire({
                ...
                onClose: () => {
                    window.location.replace('confirmCoptic.php');
                }
            }).then((result) => {
